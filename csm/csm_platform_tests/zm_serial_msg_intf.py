...
specified in KT-957-0143-00. Implements the Zeroise Micrcontroller command set for
communicating with the Zeroise Microcontroller Firmware, KT-956-0222-00.
"""
# -----------------------------------------------------------------------------
...
class ZmSerialMsgInterface:
    ...
    def send_ping(self, wait_for_ack=True):
        ...
        if wait_for_ack:
            return self.wait_for_ack(seq_no, ZmMsgId.PING.value)
        else:
            return True
    ...
    def get_command(self, get_cmd, resp_payload_len, wait_for_ack=True):
        ...
        """
        if get_cmd not in ZmMsgId:
            raise ValueError("get_cmd must be one of ZmMsgId enumerated values")
        if resp_payload_len not in ZmMsgPayloadLen:
    ...
    def send_set_buzzer_pattern(self, pattern="OFF", wait_for_ack=True):
        ...
        :param pattern: dictionary key for self.LED_PATTERNS :type: string
        :return: True if command sent and Ack received, else False
        """
        if pattern not in self.LED_PATTERNS.keys():
            return
        ...
        message_bytes = self.smh.build_message_header(seq_no=seq_no,
                                                      ...
                                                      msg_id=ZmMsgId.SET_BUZZER_PATTERN.value,
                                                      pl_len=7)

        # Build payload to set buzzer to required pattern
        message_bytes.append(0)                                 # Payload Format Version
        for entry in self.LED_PATTERNS.get(pattern):            # Pattern data
        ...
        if wait_for_ack:
            return self.wait_for_ack(seq_no, ZmMsgId.SET_BUZZER_PATTERN.value)
        else:
            return True
    ...
    def send_set_led_pattern(self, led=0, pattern="OFF", wait_for_ack=True):
        ...
        message_bytes = self.smh.build_message_header(seq_no=seq_no,
                                                      ack_no=0,
                                                      status=smh.MessageStatus.NEW_MESSAGE.value,
                                                      msg_id=ZmMsgId.SET_LED_PATTERN.value,
                                                      pl_len=9)
        ...
        self.smh.send_to_tx_queue(message_bytes)
        if wait_for_ack:
            return self.wait_for_ack(seq_no, ZmMsgId.SET_LED_PATTERN.value)
        else:
            return True
    ...
    def send_set_all_leds(self, led_patterns, wait_for_ack=True):
        ...
        :param wait_for_ack: wait for command to be acknowledged, default is True, wait for Ack :type: Boolean
        :param led_patterns: 20-element List of dictionary keys from self.LED_PATTERNS :type: list of 20 Strings
        :return: True if command sent and Ack received, else False
        """
    ...
    def send_set_led_brightness(self, brightness, wait_for_ack=True):
        ...
        message_bytes = self.smh.build_message_header(seq_no=seq_no,
                                                      ack_no=0,
                                                      status=smh.MessageStatus.NEW_MESSAGE.value,
                                                      msg_id=ZmMsgId.SET_LED_BRIGHTNESS.value,
                                                      pl_len=ZmMsgPayloadLen.SET_LED_BRIGHTNESS.value)
        ...
        self.smh.send_to_tx_queue(message_bytes)
        if wait_for_ack:
            return self.wait_for_ack(seq_no, ZmMsgId.SET_LED_BRIGHTNESS.value)
        else:
            return True
    ...
    def unpack_button_status_message(ba):
        ...
        if ba[smh.HeaderOffset.MESSAGE_ID.value] == ZmMsgId.BUTTON_STATUS.value:
    ...
    def unpack_get_software_version_number_response(ba):
        ...
        if type(ba) != bytearray or len(ba) != \
                smh.TOTAL_HEADER_LENGTH + ZmMsgPayloadLen.GET_SOFTWARE_VERSION_NUMBER.value + smh.CRC_LENGTH:
