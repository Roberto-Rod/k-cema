#!/usr/bin/env python3
"""
Utility module for testing the Keypad on a CSM Unit.
"""
# -----------------------------------------------------------------------------
...
# Our own imports ---------------------------------------------------
from zm_serial_msg_intf import ZmSerialMsgInterface, ButtonId
...
def main(kw_args):
    ...
    :return: N/A
    """
    with ZmSerialMsgInterface(kw_args.uart) as zsmi:
        if vars(kw_args).get("buzzer_state", None) is not None:
            if zsmi.send_set_buzzer_pattern("ON" if int(kw_args.buzzer_state) else "OFF", wait_for_ack=False):
        ...
        if vars(kw_args).get("led_state", None) is not None:
            zsmi.send_set_led_brightness(255, wait_for_ack=False)
            if kw_args.led_state == "ON":
                # Turn all the LEDs on Green
                for i in range(0, 20):
                    zsmi.send_set_led_pattern(led=i, pattern="OFF", wait_for_ack=False)
                for i in range(0, 20, 2):
                    zsmi.send_set_led_pattern(led=i, pattern="ON", wait_for_ack=False)
                for i in range(0, 6):
                ...
                for i in range(0, 20):
                    zsmi.send_set_led_pattern(led=i, pattern="OFF", wait_for_ack=False)
                for i in range(0, 20, 2):
                    zsmi.send_set_led_pattern(led=i, pattern="BLINK_ONE_HZ", wait_for_ack=False)
                for i in range(0, 6):
                    zsmi.send_ping(wait_for_ack=False)
                    time.sleep(1)
                    ...
                    if rx_msg:
                        ...
                        if button_status_msg:
                            for button in button_status:
                                if button.get("button_id") == getattr(ButtonId, kw_args.button).value and \
                                        button.get("button_state"):
                                    log.debug("{} button pressed".format(kw_args.button))
                                    button_pressed = True

                                if button.get("button_id") == getattr(ButtonId, kw_args.button).value and not\
                                        button.get("button_state"):
