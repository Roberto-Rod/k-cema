...
specified in KT-957-0143-00. Implements the Integrated CTS command set for
communicating with the Integrated Firmware, KT-956-0265-00.
"""
# -----------------------------------------------------------------------------
...
class CtsMsgId(Enum):
    ...
    GET_UNIT_INFO = 0xB
    SET_UNIT_INFO = 0xC
    START_SCAN_MSG_ID = 0xD
    STOP_SCAN_MSG_ID = 0xE
    GET_SCAN_STATUS_MSG_ID = 0xF
    TEST_MSG_ID = 0x10
...
class CtsMsgInterface:
    ...
    def send_start_file_upload(self, file_size_bytes, file_crc16):
        """
        ...
        return self.wait_for_ack(seq_no, CtsMsgId.TEST_MSG_ID.value)
    ...
    def verify_file_crc(self, file_crc16):
        ...
        # Wait for Ack, if it's successful wait for response
        if not self.wait_for_ack(seq_no, CtsMsgId.TEST_MSG_ID.value, verbose=True):
            log.debug("No Ack")
            return False, None
        else:
            log.debug("Ack'd")
        ...
        while True:
            ...
            if rx_msg:
                ...
                if len(rx_msg) == \
                    ...
                    if rx_msg[smh.HeaderOffset.ACKNOWLEDGEMENT_NO.value] == seq_no and \
                            rx_msg[smh.HeaderOffset.MESSAGE_ID.value] == CtsMsgId.TEST_MSG_ID.value and \
                            (((rx_msg[smh.HeaderOffset.MESSAGE_STATUS_PROTOCOL_VERSION.value] & 0xE0) == 0x80) or
                             ((rx_msg[smh.HeaderOffset.MESSAGE_STATUS_PROTOCOL_VERSION.value] & 0xE0) == 0x00)):
                        ret_val = True
                        break
                        ...
                        log.debug("Verify File CRC Rx Msg Failed Consistency Check! {} - {}; {} - {}; {}".format(
                            rx_msg[smh.HeaderOffset.ACKNOWLEDGEMENT_NO.value], seq_no,
                            rx_msg[smh.HeaderOffset.MESSAGE_ID.value], CtsMsgId.TEST_MSG_ID.value,
                            rx_msg[smh.HeaderOffset.MESSAGE_STATUS_PROTOCOL_VERSION.value] & 0xE0))
                else:
                    log.debug("Verify File CRC Rx Msg Failed Length Check!")

            if time.time() > rx_timeout:
                log.debug("Verify File CRC Rx Msg Timed Out! - {:02x}".format(CtsMsgId.TEST_MSG_ID.value))
                break
    ...
    def send_set_rf_cal_table(self, rf_cal_power_0dbm1, slope_0dp1_points, offset_mv_points, check_resp_nok=False):
        """
        ...
        :param check_resp_nok: check a Response NOK isn't sent by the receiver.
        ...
        seq_no = self._smh.get_next_sequence_number()
        cmd_id = CtsMsgId.TEST_MSG_ID.value
        header_bytes = self._smh.build_message_header(seq_no,
                                                      0,
                                                      smh.MessageStatus.NEW_MESSAGE.value,
                                                      cmd_id,
        ...
        if ret_val and check_resp_nok:
            ret_val = self.check_for_resp_nok(seq_no, cmd_id)

        return ret_val

    def send_start_scan(self, mode, freq_khz, dwell_time_ms, tx_atten_0db5, rx_atten_0db5, check_resp_nok=False):
        """
        Send a StartScan Message.
        Not handling blanking pattern at the moment.
        :return: True if message ack'd by the receiver, else False
        """
        seq_no = self._smh.get_next_sequence_number()
        cmd_id = CtsMsgId.START_SCAN_MSG_ID.value
        header_bytes = self._smh.build_message_header(seq_no,
                                                      0,
                                                      smh.MessageStatus.NEW_MESSAGE.value,
                                                      cmd_id,
                                                      CtsMsgPayloadLen.START_SCAN_MIN.value)
        ...
        payload_bytes = bytearray(struct.pack("<BBLHBBB",
                                              ...
                                              0))               # No blanking pattern transitions - i.e. None
        # Add the payload CRC
        payload_crc = CRCCCITT(version="FFFF").calculate(bytes(payload_bytes))
        payload_crc_bytes = payload_crc.to_bytes(2, byteorder="little")
        payload_bytes.append(payload_crc_bytes[0])  # CRC LSB
    ...
    def send_stop_scan(self):
        ...
        header_bytes = self._smh.build_message_header(
            ...
            ack_no=0,  # '0' for new message
            status=0,  # Status = '0' for new message and Protocol = '0'
            msg_id=CtsMsgId.STOP_SCAN_MSG_ID.value,
            pl_len=0)  # No payload

        log.debug("Tx StopScan: {}".format(" ".join(format(x, '02x') for x in header_bytes)))
        self._smh.send_to_tx_queue(header_bytes)

        return self.wait_for_ack(seq_no, CtsMsgId.STOP_SCAN_MSG_ID.value)
    ...
    def check_for_resp_nok(self, seq_no, msg_id, timeout_s=DEFAULT_RESPONSE_TIMEOUT):
        """
        Some commands will send a Response NOK after the ACK if payload processing fails.
        This method can be used to check for Response NOK messages if required.
        ...
        :return: True if the Response NOK is NOT received before timeout, else False
        """
        ret_val = True
        ...
        while True:
            ...
            rx_msg = self._smh.get_from_rx_queue()
            if rx_msg:
                log.debug("Looking for Resp NOK Msg: {}".format(" ".join(format(x, '02x') for x in rx_msg)))

                if rx_msg[smh.HeaderOffset.ACKNOWLEDGEMENT_NO.value] == seq_no and \
                        rx_msg[smh.HeaderOffset.MESSAGE_ID.value] == msg_id and \
                        ((rx_msg[smh.HeaderOffset.MESSAGE_STATUS_PROTOCOL_VERSION.value] & 0xE0) == 0xA0):
                    ret_val = False
                    break
                else:
                    log.debug("Wait for Ack Rx Msg Failed Consistency Check! {} - {}; {} - {}; {}".format(
            ...
            if time.time() > rx_timeout:
                log.debug("Looking for Resp NOK Rx Msg Timed Out! - {:02x}".format(msg_id))
                break
...
class CtsSerialTcpMsgInterface(CtsMsgInterface):
    ...
    """
    def __init__(self, host, port, response_timeout=DEFAULT_RESPONSE_TIMEOUT):
        """
        Class constructor
