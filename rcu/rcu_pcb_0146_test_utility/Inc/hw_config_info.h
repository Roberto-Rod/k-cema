/****************************************************************************/
/**
** Copyright 2020 Kirintec Ltd. All rights reserved.
**
** @file hw_config_info.h.h
**
** Include file for hw_config_info.c
**
** Project : K-CEMA
**
** Build instructions : None, include file only
**
****************************************************************************/

/* Define to prevent recursive inclusion */
#ifndef __HW_CONFIG_INFO_H
#define __HW_CONFIG_INFO_H

/*****************************************************************************
*
*  Include
*
*****************************************************************************/
#include "stm32l0xx_hal.h"
#include <stdbool.h>

/*****************************************************************************
*
*  Global Definitions
*
*****************************************************************************/
#define PCA9500_EEPROM_I2C_ADDR 0x52U << 1
#define PCA9500_GPIO_I2C_ADDR	0x22U << 1
#define PCA9500_MEM_SIZE_BYTES	256U
#define PCA9500_PAGE_SIZE_BYES	4U
#define PCAA9500_WRITE_TIME_MS	10U
#define I2C_TIMEOUT				100U
#define HCI_STR_PARAM_LEN		16

/*****************************************************************************
*
*  Global Macros
*
*****************************************************************************/


/*****************************************************************************
*
*  Global Datatypes
*
*****************************************************************************/
typedef struct
{
	uint8_t 	assy_part_no[HCI_STR_PARAM_LEN];
	uint8_t 	assy_rev_no[HCI_STR_PARAM_LEN];
	uint8_t		assy_serial_no[HCI_STR_PARAM_LEN];
	uint8_t 	assy_build_date_batch_no[HCI_STR_PARAM_LEN];
	uint8_t 	hci_version_no;
	uint16_t	hci_crc;
	bool		hci_crc_valid;
	uint8_t		hw_version;
	uint8_t 	hw_mod_version;
} hci_HwConfigInfo;

typedef struct
{
	uint8_t 	assy_part_no[HCI_STR_PARAM_LEN];
	uint8_t 	assy_rev_no[HCI_STR_PARAM_LEN];
	uint8_t		assy_serial_no[HCI_STR_PARAM_LEN];
	uint8_t 	assy_build_date_batch_no[HCI_STR_PARAM_LEN];
	uint8_t		spare[189];	/* Pad data structure to 256-bytes */
	uint8_t 	hci_version_no;
	uint16_t	hci_crc;
} hci_HwConfigEepromData;

/*****************************************************************************
*
*  Global Functions
*
*****************************************************************************/
bool hci_ReadHwConfigInfo(	I2C_HandleTypeDef* 	i2c_device,
							hci_HwConfigInfo* 	p_hw_config_info);
bool hci_ResetHwConfigInfo(I2C_HandleTypeDef* i2c_device);
bool hci_SetAssyPartNo(I2C_HandleTypeDef* i2c_device, uint8_t* assy_part_no);
bool hci_SetAssyRevNo(I2C_HandleTypeDef* i2c_device, uint8_t* assy_rev_no);
bool hci_SetAssySerialNo(I2C_HandleTypeDef* i2c_device, uint8_t* assy_serial_no);
bool hci_SetAssyBuildDataBatchNo(	I2C_HandleTypeDef* i2c_device,
									uint8_t* assy_build_date_batch_no);

/*****************************************************************************
*
*  External Variables
*
*****************************************************************************/


/*****************************************************************************
*
*  Local to the C file
*
*****************************************************************************/
#ifdef __HW_CONFIG_INFO_C

/*****************************************************************************
*
*  Local Definitions
*
*****************************************************************************/


/*****************************************************************************
*
*  Local Datatypes
*
*****************************************************************************/
#define HCI_CRC_LEN		2U

/*****************************************************************************
*
*  Local Functions
*
*****************************************************************************/
bool hci_WriteDeviceData(	I2C_HandleTypeDef* i2c_device,
							hci_HwConfigEepromData* device_data);
uint16_t hci_ComputeCRCCCITT(uint8_t* message, uint16_t msg_length);

/*****************************************************************************
*
*  Local Variables
*
*****************************************************************************/
hci_HwConfigInfo lg_hci_data= {0U};
hci_HwConfigEepromData lg_device_data;

#endif /* __HW_CONFIG_INFO_C */

#endif /* __HW_CONFIG_INFO_H */
