...
/*****************************************************************************/
/**
* Serial buffer receive task function.
*
...
*
******************************************************************************/
void sbt_SerialBufferRxTask(void const *argument)
{
	...
	for(;;)
	{
		event = osMessageGet(lg_sbt_init_data.rx_event_queue, osWaitForever);

		p_event_buf = (sbt_Event_t *)&event.value.v;
		osMessagePut(lg_sbt_init_data.uarts[p_event_buf->uart_idx].uart_rx_data_queue, (uint32_t)p_event_buf->data, 1U);
	}
}
...
/*****************************************************************************/
/**
* Serial buffer transmit task function.
*
* @param    argument not used
* @return   None
* @note     None
*
******************************************************************************/
void sbt_SerialBufferTxTask(void const *argument)
{
	int16_t i = 0;
							...
							uint8_t*			uart_tx_buf)
{
	...
	/*  If the UART is not busy transmitting and there are messages to send */
	if (!(HAL_UART_GetState(huart) & 0x1U) && (tx_count > 0U))
	{
