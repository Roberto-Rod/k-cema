...
def run_test(assembly_type, zero_serial_number=False, config_version=1, revision=None, serial_number=None, date=None):
    ...
    if not 0 < config_version <= len(option_list):
        print("Invalid hardware confoig version requested (requested {}, max available is {})".format(config_version, len(option_list)))
        return False
...
if __name__ == "__main__":
    if run_test(assembly_type=AssemblyType.EMA_HB_R):
        print("\n*** OK - test passed ***\n")
    else:
        print("\n*** TEST FAILED ***\n")
