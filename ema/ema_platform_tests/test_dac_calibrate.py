...
def run_test(band, zero_power_meter=True):
    ...
    for freq in range(freq_start_MHz[band], freq_stop_MHz[band] + freq_step_MHz[band], freq_step_MHz[band]):
        # Set frequency
        print("Set DAC to {} MHz: ".format(freq), end="")
        dac.set_frequency(freq * 1e6)
        print("OK")
        ...
        if cal_point_found:
            amplitude = dac.get_amplitude()
            print("OK [att = {} dB, ASF = {}]".format(att_dB, amplitude))
            try:
                # Write to file, frequency in Hz
                f.write("{:.0f},{}\n".format(freq * 1e6, amplitude))
            except:
