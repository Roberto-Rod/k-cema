...
bool igd_Init(igd_I2cGpioDriver_t *p_inst)
{
	...
	if (igd_WriteRegister(p_inst, IGD_MCP23017_OLAT_REG_ADDR, p_inst->default_op_mask))
	{
		...
		if (igd_WriteRegister(p_inst, IGD_MCP23017_IODIR_REG_ADDR, p_inst->io_dir_mask))
		{
			/* Set IO pull-up states */
			ret_val = 	igd_WriteRegister(p_inst, IGD_MCP23017_GPPU_REG_ADDR, p_inst->io_pu_mask);
		}
	}
