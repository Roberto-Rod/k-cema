...
class RFControl:
    ...
    RF_CONTROL_TX_EHB_PATH_MASK = 0x00000070
    ...
    @staticmethod
    def set_tx_path(path, band):
        if True:
            # Modify path for control of -0202- board (write path straight through)
            pass
        else:
            if band == Band.HIGH:
                ...
                if path <= 2:
                    ...
                    path += 6
        DevMem.rmw(RFControl.REG_RF_CONTROL, path, RFControl.RF_CONTROL_TX_EHB_PATH_MASK)

    @staticmethod
    def set_synth_path(path):
        if path == 0:
