...
# CONSTANTS
# -----------------------------------------------------------------------------
ASSEMBLY_NUMBER = {AssemblyType.CSM_MOTHERBOARD: "KT-000-0180-00", AssemblyType.CSM_ASSEMBLY: "KT-950-0351-00"}
...
def get_config_info(assembly_type, config_version=1):
    ...
    if assembly_type == AssemblyType.CSM_MOTHERBOARD:
...
def refresh_config_info():
    ...
    """
    success, return_dict = get_config_info(AssemblyType.CSM_MOTHERBOARD)
    ...
    if success:
        if set_config_info(return_dict.get("Assembly Serial Number"),
                           ...
                           AssemblyType.CSM_MOTHERBOARD):
            log.info("Hardware config info successfully refreshed")
    else:
        log.critical("Set hardware config info failed!")

    success, return_dict = get_config_info(AssemblyType.CSM_ASSEMBLY)

    if success:
        if set_config_info(return_dict.get("Assembly Serial Number"),
                           return_dict.get("Assembly Revision Number"),
                           return_dict.get("Assembly Build Date/Batch Number"),
                           AssemblyType.CSM_ASSEMBLY):
            log.info("Unit config info successfully refreshed")
    else:
        log.critical("Set unit config info failed!")
...
if __name__ == '__main__':
    ...
    parser.add_argument("-st", "--set_assy_type", default="",
                        help="Set assembly type: CSM_ASSEMBLY or CSM_MOTHERBOARD")
    ...
    if args.set_batch_build_no != "" and args.set_serial_no != "" and \
        ...
        if args.set_assy_type == "CSM_ASSEMBLY" or args.set_assy_type == "CSM_MOTHERBOARD":
            ...
            log.info("INFO - Invalid assembly type {}, must be CSM_ASSEMBLY or CSM_MOTHERBOARD"
