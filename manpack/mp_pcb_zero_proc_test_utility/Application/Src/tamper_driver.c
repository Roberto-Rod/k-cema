						...
						bool tpm, bool tcm, bool enable)
{
		...
		((channel >= td_TamperChannel1) && (channel <= td_TamperChannel2)))
	{
		...
		{
			/* Leave the tamper channels inactive as expected by the BootBlocker. */
			/* Channel 1: Normally Closed/Tamper High; Clear RAM on Tamper; 1 MOhm pull-down */
			/* Channel 2: Normally Open/Tamper Low; Clear RAM on Tamper; 1 MOhm pull-up */
			if (channel == td_TamperChannel1)
			{
				buf = 0x15U;
			}
			else
