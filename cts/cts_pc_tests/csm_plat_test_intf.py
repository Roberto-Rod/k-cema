...
- CSM Platform Test Script installed in folder "/run/media/mmcblk1p2/test/" on
  the KT-000-0140-00 board.

Software compatibility:
- KT-956-0234-00 K-CEMA CSM Platform Test Scripts V2.0.0
"""
# -----------------------------------------------------------------------------
...
class CsmPlatformTest:
    ...
    ASSERT_RF_MUTE_CMD = "ext_rf_mute.py 1"
    ASSERT_RF_MUTE_CMD_SUCCESS = "RF_MUTE - Asserted"
    DEASSERT_RF_MUTE_CMD = "ext_rf_mute.py 0"
    DEASSERT_RF_MUTE_CMD_SUCCESS = "RF_MUTE - De-asserted"
    ...
    GET_LWIP_ADDRESSES_CMD = "gbe_switch.py -c /dev/ttyEthSw -l"
    ...
    def set_config_info(self, assy_type, assy_rev_no, assy_serial_no, assy_batch_no):
        ...
        if self.check_ssh_connection():
            if assy_type == self.CSM_ASSEMBLY_NO or assy_type == self.CSM_MOTHERBOARD_NO:
                at = "CSM_ASSEMBLY" if assy_type == self.CSM_ASSEMBLY_NO else "CSM_MOTHERBOARD"
                cmd_str = "{} {}{}".format(self.PYTHON_PATH, self.CSM_TEST_SCRIPT_PATH,
    ...
    def remove_test_scripts(self):
        ...
        if self.check_ssh_connection():
            remove_test_script_cmds = [
                ...
                ("rm -rfv /run/media/mmcblk1p2/test;rm -d /run/media/mmcblk1p2/test;/sbin/reboot -f",
    ...
    def copy_test_scripts_to_som(self, test_script_archive):
        ...
        if self.check_ssh_connection():
            copy_test_script_cmds = [
                # (command, response1, response2)
                ("cd /run/media/mmcblk1p2;ls;rm -rf test;/bin/tar -xvzf csm_p2.tgz", "test/", "")
            ]
    ...
    def get_lwip_addresses(self):
        """
        :return: Returns a list of LwIP addresses connected to the GbE Switch
        """
        lwip_addresses = []

        if self.check_ssh_connection():
            cmd_str = "{} {}{}".format(self.PYTHON_PATH, self.CSM_TEST_SCRIPT_PATH, self.GET_LWIP_ADDRESSES_CMD)
            resp = self._ssh_conn.send_command(cmd_str)

            for a_line in resp.stderr.splitlines():
                if "Found the following LWIP AUTOIP addresses: " in a_line:
                    addr_list = a_line[a_line.find('[')+1:a_line.find(']')]
                    addr_list = addr_list.strip('\'')
                    lwip_addresses = addr_list.split(',')
                    break

        return lwip_addresses
