#!/usr/bin/env python3
"""
Keysight HP83752A Signal Generator Class
...
class SignalGeneratorHP83752A():
    def __init__(self, debug = False):
        super().__init__()
        log.info("INFO - Instanciating Keysight HP83752A Signal Generator Class!")
        ...
        for res in self.rm.list_resources():
            if res:
                ...
                if self.initialise_device():
                    log.info("INFO - Found and initialised HP83752A Signal Generator: {}".format(self.details()))
                    return True
                else:
                    self.resource = None
        log.info("ERROR - did not find an HP83752A Signal Generator")
        return False
            
    def initialise_device(self):
        try:
            self.sg = self.rm.open_resource(self.resource)            
            if "83752A" in self.details():
    ...
    def set_output_power_dBm(self, power_dBm):
        if self.send_command("POW {:.2f} DBM".format(power_dBm)):
            return self.wait_command_complete()
        else:
            return False
    
    def get_output_power_dBm(self):
        return float(self.send_query("POW?").strip())
    
    def set_output_enable(self, enable_state):
        if self.send_command("POW:STAT ON" if enable_state else "POW:STAT OFF"):
            return self.wait_command_complete()
        else:
            return False
    
    def get_output_enable(self):
        resp = self.send_query("POW:STAT?").strip()
...
def test():
    sg = SignalGeneratorHP83752A()
    log.info("INFO - SignalGeneratorHP83752A Test:")
