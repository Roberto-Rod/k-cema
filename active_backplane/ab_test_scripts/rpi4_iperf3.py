...
def start_iperf3_server():
    with SSH(RPI4_HOSTNAME, RPI4_USERNAME, RPI4_PASSWORD) as rpi4:
        # Ensure any running iperf3 instances are killed
        rpi4.send_command("killall -9 iperf3")

        # Start iperf3 running in server mode as a daemon mode
        rpi4.send_command("iperf3 -s -D -i 1")

        # Check the iperf3 server is running
        return rpi4.send_command("ps aux | grep iperf3").stdout.find(IPERF3_START_SERVER_CMD) != -1
...
def stop_iperf3_server():
    with SSH(RPI4_HOSTNAME, RPI4_USERNAME, RPI4_PASSWORD) as rpi4:
        ...
        return rpi4.send_command("ps aux | grep iperf3").stdout.find(IPERF3_DETECT_SERVER) == -1
