#!/usr/bin/env python3
"""
Class that encapsulates the interface to the KT-956-0266-00 software running
on the KT-000-0199-00 Fill Device Motherboard STM32 microcontroller.

Software compatibility:
- KT-956-0266-00 K-CEMA DRCU Micro Test Utility - v1.0.0 onwards
...
class FdTamperChannels(Enum):
    CHANNEL_0 = 0
    CHANNEL_1 = 1


class FdTamperChannelStatus(Enum):
    DISABLED = 0
    ARMED_READY = 1
    TAMPERED = 2
    UNKNOWN = 3
...
class FdMircoTestInterface:
    ...
    _GET_ADC_DATA_TEMP_CMD = b"$ADC"
    _GET_ADC_DATA_RESPONSE_END = b"!ADC\r\n"
    _SET_GPO_CMD = b"#GPO"
    ...
    def get_poe_pd_pse_type(self):
        ...
        """
        ret_val = False
        pse_type = FdPoEPseType.INVALID_0
    ...
    def set_gpo_signal(self, gpo_signal, state):
        ...
        """
        if gpo_signal not in FdGpoSignals:
    ...
    def set_anti_tamper_channel_enable(self, channel, enable):
        ...
        """
        if channel not in FdTamperChannels:
    ...
    def get_tamper_channel_status(self, channel):
        ...
        ret_val = False
        status = FdTamperChannelStatus.UNKNOWN
        ...
        if self._serial_port is not None:
            ...
            if self._GET_TAMPER_RESPONSE_END in resp_str:
                ...
                if channel is FdTamperChannels.CHANNEL_0:
                    tamper_reg = b"Anti-tamper Tamper 1"
                    flags_reg = b"Anti-tamper Flags"
                    tb_bit = 0x02
                elif channel is FdTamperChannels.CHANNEL_1:
                ...
                if teb_reg < 256 and tb_reg < 256:
                    if teb_reg & 0x80 == 0:
                        status = FdTamperChannelStatus.DISABLED
                    elif tb_reg & tb_bit:
                        status = FdTamperChannelStatus.TAMPERED
                    else:
                        status = FdTamperChannelStatus.ARMED_READY
                    ret_val = True
        else:
            raise RuntimeError("Serial port is not open!")
    ...
    def get_vbat_monitor_mv(self):
        """
        Read the ADC data, extract and return the VBAT monitor reading.
        :return: VBAT voltage, resolution mV :type Integer
        """
        vbat_mv = -16535

        if self._serial_port is not None:
            self._serial_port.write(self._GET_ADC_DATA_CMD + self._SET_CMD_END)
            resp_str = self._serial_port.read_until(self._GET_ADC_DATA_RESPONSE_END)

            # Was the command response terminator found or did it timeout?
            if self._GET_ADC_DATA_RESPONSE_END in resp_str:
                for a_line in resp_str.splitlines():
                    if b"PVBAT (mV):" in a_line:
                        vbat_mv = int(a_line.split()[-1])
        else:
            raise RuntimeError("Serial port is not open!")
