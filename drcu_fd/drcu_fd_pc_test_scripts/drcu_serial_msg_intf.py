...
specified in KT-957-0143-00. Implements the Display RCU Micrcontroller command set
for communicating with the Display Microcontroller Firmware, KT-956-0376-00.
"""
# -----------------------------------------------------------------------------
...
class DrcuButtonStatusOffset(Enum):
...
class DrcuLedPattern:
    led_pattern: int = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00]


class DrcuSerialMsgInterface:
    """
    Class for handling serial messages to the zeroise microcontroller firmware
    firmware KT-956-0222-00.  The serial message ICD is defined in document
    KT-957-0413-00
    """
    ...
    def wait_for_ack(self, ack_no, msg_id):
        ...
        :return: True if the ack received before DRCU_SERIAL_TIMEOUT, else False
        """
        ret_val = False
        rx_timeout = time.time() + DRCU_SERIAL_TIMEOUT
        while True:
            time.sleep(0.001)
    ...
    def send_ping(self, wait_for_ack=True):
        ...
        if wait_for_ack:
            return self.wait_for_ack(seq_no, DrcuMsgId.PING.value)
        else:
            return True
    ...
    def get_command(self, get_cmd, resp_payload_len, wait_for_ack=True):
        ...
        """
        if get_cmd not in DrcuMsgId:
            raise ValueError("get_cmd must be one of ZmMsgId enumerated values")
        if resp_payload_len not in DrcuMsgPayloadLen:
    ...
    def send_set_buzzer_pattern(self, pattern="OFF", wait_for_ack=True):
        """
        Constructs and sends a SetBuzzerPattern message
        :param wait_for_ack: wait for command to be acknowledged, default is True, wait for Ack :type: Boolean
        :param pattern: dictionary key for DRCU_LED_PATTERNS :type: string
        :return: True if command sent and Ack received, else False
        """
        if pattern not in DRCU_LED_PATTERNS.keys():
            return False
        ...
        message_bytes = self.smh.build_message_header(seq_no=seq_no,
                                                      ...
                                                      msg_id=DrcuMsgId.SET_BUZZER_PATTERN.value,
                                                      pl_len=7)

        # Build payload to set buzzer to required pattern
        message_bytes.append(0)                                 # Payload Format Version
        for entry in DRCU_LED_PATTERNS.get(pattern):            # Pattern data
        ...
        log.debug("Tx SetBuzzerPattern: {}".format(" ".join(format(x, '02x') for x in message_bytes)))
        self.smh.send_to_tx_queue(message_bytes)
        if wait_for_ack:
            return self.wait_for_ack(seq_no, DrcuMsgId.SET_BUZZER_PATTERN.value)
        else:
            return True
    ...
    def send_set_led_pattern(self, led=0, pattern="OFF", wait_for_ack=True):
        ...
        message_bytes = self.smh.build_message_header(seq_no=seq_no,
                                                      ack_no=0,
                                                      status=smh.MessageStatus.NEW_MESSAGE.value,
                                                      msg_id=DrcuMsgId.SET_LED_PATTERN.value,
                                                      pl_len=9)
        ...
        self.smh.send_to_tx_queue(message_bytes)
        if wait_for_ack:
            return self.wait_for_ack(seq_no, DrcuMsgId.SET_LED_PATTERN.value)
        else:
            return True

    def send_set_all_leds(self, led_patterns, wait_for_ack=True):
        """
        Constructs and sends a SetLedPattern that sets all 20x LEDs
        :param wait_for_ack: wait for command to be acknowledged, default is True, wait for Ack :type: Boolean
        :param led_patterns: 20-element List of dictionary keys from DRCU_LED_PATTERNS :type: list of 20 Strings
        :return: True if command sent and Ack received, else False
        """
        if len(led_patterns) != 20:
            return False
        for pattern in led_patterns:
            if pattern not in DRCU_LED_PATTERNS:
                return False
        # Build the message header
        seq_no = self.smh.get_next_sequence_number()
        message_bytes = self.smh.build_message_header(seq_no=seq_no,
        ...
        # Build payload to set LED to required pattern
        message_bytes.append(0)     # Payload Format Version
        message_bytes.append(20)    # Number of Patterns, 20 all LEDs
    ...
    def send_set_led_brightness(self, brightness, wait_for_ack=True):
        ...
        message_bytes = self.smh.build_message_header(seq_no=seq_no,
                                                      ack_no=0,
                                                      status=smh.MessageStatus.NEW_MESSAGE.value,
                                                      msg_id=DrcuMsgId.SET_LED_BRIGHTNESS.value,
                                                      pl_len=DrcuMsgPayloadLen.SET_LED_BRIGHTNESS.value)
        ...
        self.smh.send_to_tx_queue(message_bytes)
        if wait_for_ack:
            return self.wait_for_ack(seq_no, DrcuMsgId.SET_LED_BRIGHTNESS.value)
        else:
            return True
    ...
    def unpack_button_status_message(ba):
        ...
        if ba[smh.HeaderOffset.MESSAGE_ID.value] == DrcuMsgId.BUTTON_STATUS.value:
            for btn_idx in range(0, ba[payload_start_idx + 1]):
                button_id = ba[payload_start_idx + 2 + (btn_idx * 3) + DrcuButtonStatusOffset.BUTTON_ID.value]
                button_state = ba[payload_start_idx + 2 + (btn_idx * 3) + DrcuButtonStatusOffset.BUTTON_STATE.value]
                button_hold_time = ba[payload_start_idx + 2 + (btn_idx * 3) + DrcuButtonStatusOffset.BUTTON_HOLD_TIME.value]
    ...
    def unpack_get_software_version_number_response(ba):
        ...
        if type(ba) != bytearray or len(ba) != \
                smh.TOTAL_HEADER_LENGTH + DrcuMsgPayloadLen.GET_SOFTWARE_VERSION_NUMBER.value + smh.CRC_LENGTH:
    ...
    def unpack_get_bit_info_response(ba):
        ...
        :param ba: ByteArray payload of response message to unpack
        :return: None if ba is wrong type, length or payload type; else dictionary of BIT information
        """
        if type(ba) != bytearray or len(ba) != \
                smh.TOTAL_HEADER_LENGTH + DrcuMsgPayloadLen.GET_BIT_INFO.value + smh.CRC_LENGTH:
            log.critical("Unpack GetBitInformation Response expecting {}-byte array".format(
                smh.TOTAL_HEADER_LENGTH + DrcuMsgPayloadLen.GET_BIT_INFO.value + smh.CRC_LENGTH))
            return None
        else:

            payload_version, status_flags, temp_0_25_deg_c, ltc6065_status = \
                struct.unpack_from("<BBhB", ba, smh.TOTAL_HEADER_LENGTH)
            # Is this a DRCU BIT information payload?
            if payload_version == 4:
                ret_data = {"invalid_temp": True if status_flags & 0x01 else False,
                            "temp": float(temp_0_25_deg_c) / 4.0,
                            "batt_chrg_stby": True if ltc6065_status & 0x01 else False,
                            "batt_data_fault": True if ltc6065_status & 0x02 else False}
                return ret_data
            else:
                return None
