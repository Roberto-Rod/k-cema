...
#define SCT_ADC_NUM_STEPS				4096
#define SCT_VDD_CALIB_MV 				((uint16_t) (3000))
#define SCT_NUM_ADC_CHANNELS			3
#define SCT_PVBAT_READING_IDX			0
#define SCT_VREFINT_READING_IDX			1
#define SCT_TEMPERATURE_READING_IDX		2
...
static void sct_ProcessGetAdcCommand(uint8_t *resp_buf);
...
static void sct_ProcessCommand(uint8_t *cmd_buf, uint8_t *resp_buf)
{
	...
	{
		sct_ProcessGetAdcCommand(resp_buf);
	}
...
/*****************************************************************************/
/**
* Read and return the internal ADC data.
*
* @param    resp_buf buffer for transmitting command response
* @todo		Should really pass the PVBAT channel information in through the
*           initialisation data.
*
******************************************************************************/
static void sct_ProcessGetAdcCommand(uint8_t *resp_buf)
{
	...
	sprintf((char *)resp_buf, "Vrefext (mV): %ld%s", vref_ext_mv, SCT_CRLF);
	sct_FlushRespBuf(resp_buf);

	/* Calculate voltages */
	pvbat_mv = ((adc_reading[SCT_PVBAT_READING_IDX] * 2 * vref_ext_mv) / SCT_ADC_NUM_STEPS);
	sprintf((char *)resp_buf, "PVBAT (mV): %ld%s", pvbat_mv, SCT_CRLF);
	sct_FlushRespBuf(resp_buf);;
	...
	sprintf((char *)resp_buf, "Temperature (deg C): %ld%s", temperature, SCT_CRLF);
	sct_FlushRespBuf(resp_buf);
